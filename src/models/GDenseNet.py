...
from src.models.keras_gcnn import GConv2D, GBatchNorm
from src.models.keras_gcnn.pooling import GroupPool
...
def __conv_block(ip, nb_filter, mc_dropout, padding, bn_momentum, use_g_bn, mc_bn, bottleneck=False, dropout_rate=None,
    ...
    with (((K.name_scope('ConvBlock')))):
        ...
        x = ip
        temp = __BatchNorm(use_g_bn, conv_group, use_gcnn, momentum=bn_momentum, epsilon=1.1e-5, axis=concat_axis,
                        name=name_or_none(block_prefix, '_bn'))
        x = temp(x, training=mc_bn)
        x = Activation('relu')(x)
